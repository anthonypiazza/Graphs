...
def path_creator_traversal():
    ...
    while s.size() > 0:
        ...
        if visited is None:        
            ...
            # print(f"Currently in Room {player.currentRoom.id}")
            visited = [] 
            visited.append(player.currentRoom.id)
            
            for direction in roomGraph[player.currentRoom.id][1]:
                # print(f"Currently in Room: {player.currentRoom.id}, pushing ({direction}) to the stack")
                s.push(direction)

        else:
            move_to_previous_room = ''
            new_rooms = []
            
            if player.currentRoom and player.currentRoom.getRoomInDirection(v):
                visited_edges.append( (player.currentRoom.id, player.currentRoom.getRoomInDirection(v).id) )
            
            
            # print(f"\n\n\nVisited: {visited}) 
            # print(f"Current Room: {player.currentRoom.id} moving {v} to Room {player.currentRoom.getRoomInDirection(v).id}")
            ...
            if player.currentRoom.id not in visited:
                ...
                if len(roomGraph[player.currentRoom.id][1]) == 1:

                    for direction in roomGraph[player.currentRoom.id][1]:
                        # print(f"Currently in Room: {player.currentRoom.id}, pushing ({direction}) to the stack") 
                        s.push(direction)

                elif len(roomGraph[player.currentRoom.id][1]) >= 2:
                    # print(f"This room has more than one exit")
                    # print(roomGraph[player.currentRoom.id][1])

                    for direction in roomGraph[player.currentRoom.id][1]:
                        if player.currentRoom.getRoomInDirection(direction).id in visited:
                            for edge_combo in visited_edges:
                                if player.currentRoom.getRoomInDirection(direction).id == edge_combo[0] and player.currentRoom.id == edge_combo[1]:
                                    move_to_previous_room = direction
                        else:
                            new_rooms.append(direction)

                    s.push(move_to_previous_room)
                    # print(f"Currently in Room: {player.currentRoom.id}, pushing ({move_to_previous_room}) to the stack\n")

                    for move in new_rooms:
                        # print(f"Currently in Room: {player.currentRoom.id}, pushing ({move}) to the stack\n")
                        s.push(move)
            else:
                new_v = s.pop()
                after_next_v = s.pop()
                # print(f"Back in room {player.currentRoom.id} going '{new_v}' if you pop this you will go {after_next_v}")
                s.push(after_next_v)
                
                if player.currentRoom.getRoomInDirection(new_v) and player.currentRoom.getRoomInDirection(new_v).id not in visited:
                    s.push(new_v)
                    # print("Dfdsfdsfsdf\n\n")
                
                elif player.currentRoom.getRoomInDirection(new_v) and player.currentRoom.getRoomInDirection(new_v).id in visited:
                    for edge_combo in visited_edges:
                        if player.currentRoom.getRoomInDirection(new_v).id == edge_combo[0] and player.currentRoom.id == edge_combo[1]:
                            # print('dfsfdsfsf\n\n')
                            s.push(new_v)
            # print(len(visited))
            # print(s.size())
...
# Logs path as player walks
traversalPath = path_creator_traversal()

# To do this, Write a traversal algorithm that logs the path into `traversalPath` as it walks.

# 1. Understand the problem
    # - Nodes: Rooms
    # - Vertices: hallways between rooms
    # - Undirected
    # - Cyclic
    # - Sparse

# Traversal -->Create directions that when walked 
# in order will visit every room on the map at least once
...
# You are done when you have exactly 500 entries (0-499) in your graph 
